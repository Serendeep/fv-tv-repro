...
def run(args):
    ...
    manifest = {'model':args.model,'seeds':SEEDS,'mappings':MAPPINGS,'split_seed':20260906,
                'dev_per_class':20,'test_per_class':40,'demos_per_class':5,'batch_size':8,
            ...
            def predict(section, variant, layer=None):
                ...
                if key not in state['predictions']:
                    ...
                    if variant in ['zero','icl']:
                        preds = eval_icl.predict_top1(model,prompts,batch_size=8,remote=True)
                    else:
                        preds = tv.patch_theta(model,prompts,vectors[variant][layer],layer,batch_size=8,remote=True)
